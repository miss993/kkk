...
var fenzuGrid = new Ext.grid.GridPanel({
			id : 'fenzuGrid',
			plugins : [rowEditing],
			store : store,
			region : 'center',
...
fenzuGrid.getSelectionModel().on('selectionchange',
		function(selModel, selections) {
			fenzuGrid.down('#delete').setDisabled(selections.length === 0);
		});
...
var clearForm = function() {
	...
	fenzuForm.getForm().reset();
}

var queryForm = function() {
	Ext.Msg.alert('查询', '将开始执行查询！');
}
var fenzuForm = new Ext.form.FormPanel({
			title : '信息查询',
			width : 200,
			height : 200,
			frame : true,
